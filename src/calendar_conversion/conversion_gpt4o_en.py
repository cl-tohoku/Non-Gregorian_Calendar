...
def generate_batch(prompts, client, model, max_new_tokens=32):
    results = []
...
def run(args):
    ...
    for path in sorted(glob.glob(f"{input_dir}/en/*.jsonl")):
        ...
        responses_wareki = generate_batch(prompts_wareki, client, model)
        responses_seireki = generate_batch(prompts_seireki, client, model)
